...
print(f'Cvičení 2\n*************************************')
# Generating a list of numbers between 1 and 2000 that are divisible by 200
hundreds = [num for num in range(1, 2001) if num % 200 == 0]
print(hundreds)

# Vygenerování 50 náhodných písmen abecedy (velká a malá písmena)
ascii_list = [random.choice(string.ascii_uppercase) for let in range(50)]
print(ascii_list)

hundreds = hundreds[3:-3]
print(hundreds)

# unique = [set(ascii_list)]
unique = [let for let in ascii_list if not ascii_list.count(let) > 1]
print(unique)

hund_ascii = list(zip(hundreds, ascii_list))
print(hund_ascii)
